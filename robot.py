...
class Robot:
    ...
    def measure_distance_and_bearing_to(self, point: Tuple, noise: bool = False):
        ...
        bearing_to_point = bearing_to_point

        if noise:
            distance_sigma = self.measure_distance_noise * max(1.0, 1 / math.exp(distance_to_point))
            bearing_sigma = self.measure_bearing_noise * max(1.0, 1 / math.exp(distance_to_point))
